...
def summarize_text_ollama(text, summary_type):
    """Generate a summary for a given text chunk using Ollama's Llama model."""
    try:
        prompt = generate_summary_prompt(text, summary_type)
        #response = ollama.chat(model='llama3.1', messages=[{'role': 'user', 'content': 'mention in response it is generated by gpt-oss: '+prompt}]) # llama3.1
        response = ollama.chat(model='gpt-oss:20b', messages=[{'role': 'user', 'content': prompt}]) # gpt-oss:20b
